...
Completed:    -       A      B        C      D,E
             ...
             [Элементы становятся в очередь]   [Обработка 2 одновременно]   [Завершение]
              при занятости                   с ожиданием между            всех элементов
```
...
- Обрабатывать задачи на основе Promise с правильной обработкой ошибок
- Сохранять порядок при максимальной пропускной способности
- Выполнять фоновые задачи, которые могут работать параллельно
...
- Обработка нескольких websocket-соединений
- Обработка потоков данных с обратным давлением (backpressure)
- Управление ресурсоемкими фоновыми задачами

### Когда не использовать асинхронную очередь (When Not to Use Async Queueing)

`AsyncQueuer` очень универсален и может использоваться во многих ситуациях. По сути, он не подходит только тогда, когда вы не планируете использовать все его возможности. Если вам не нужно, чтобы все поставленные в очередь выполнения проходили через него, используйте [Throttling][../guides/throttling]. Если вам не нужна параллельная обработка, используйте [Queueing][../guides/queueing].
...
`AsyncQueuer` поддерживает различные стратегии очередей для обработки различных требований. Каждая стратегия определяет, как задачи добавляются и обрабатываются из очереди.
...
// Обработка: first и second параллельно
```

#### Стек LIFO (Last In, First Out)

Стеки LIFO обрабатывают последние добавленные задачи первыми, что полезно для приоритизации новых задач:
...
// Обработка: second сначала, затем first
```
...
// Добавляем задачи в любом порядке - они будут обрабатываться по приоритету (сначала большие числа)
priorityQueue.addItem(lowPriorityTask)
priorityQueue.addItem(highPriorityTask)
priorityQueue.addItem(mediumPriorityTask)
// Обработка: high и medium параллельно, затем low
```
...
const dynamicPriorityQueue = new AsyncQueuer<string>({
  ...
  getPriority: (task) => {
    ...
    // Большие числа имеют больший приоритет
    return calculateTaskPriority(task)
  }
})

// Добавляем задачи - приоритет будет рассчитываться динамически
...
- Нужен гибкий порядок задач на основе приоритета
...
`AsyncQueuer` предоставляет несколько методов для мониторинга и контроля состояния очереди:
...
Параметры `AsyncQueuer` могут быть изменены после создания с помощью `setOptions()` и получены с помощью `getOptions()`:
...
Каждый адаптер для фреймворка создает удобные хуки и функции вокруг классов асинхронной очереди. Хуки типа `useAsyncQueuer` или `useAsyncQueuerState` — это небольшие обертки, которые могут сократить шаблонный код в вашем приложении для некоторых распространенных случаев использования.
